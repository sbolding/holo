//
//
//  Generated by StarUML(tm) C++ Add-In
//
//  @ Project : Untitled
//  @ File Name : FluxElementTally.cpp
//  @ Date : 1/31/2014
//  @ Author : 
//
//


#include "FluxElementTally.h"

FluxElementTally::FluxElementTally(int n_angular_bins, int n_spatial_moments):
	ElementTally(n_angular_bins, n_spatial_moments)
{}

void FluxElementTally::incrementScore(double weight, double path_length,
	double normal_cosine, double volume, double normalized_position)
{
	double value;  //tally value
	//Determine which bin you are in (currently it assumes only two)
	if (_bin_sums.size() == 2)
	{
		int angular_bin = floor(normal_cosine) + 1; //either 0 or 1

		//loop over spatial moments, recursively, zeroth bin is 0th moment
		value = weight*path_length / volume; //0th spatial moment
		if (value < 0.)
		{
			std::cerr << "You have passed in a negative pathlength to the tally";
			exit(1);
		}
		for (int i = 0; i < _bin_sums[0].size(); ++i)
		{
			_bin_sums[angular_bin][i] += value;
			_bin_sums_sq[angular_bin][i] += value*value;
			value *= normalized_position; //get spatial moment for next iterat.
		}
	}
	else
	{
		std::cerr << "Tallies only handle two directions currently \n";
		exit(29);
	}

}
